 $(function(){
	...
	$.ajax({
		...
		success: function(data) {
			...
			var city_name = $(data).find('name:eq(1)').text();
			var country = $(data).find('country:eq(1)').text();
			var province = $(data).find('province:eq(1)').text();
			var image = $(data).find('image:eq(2)').text();
			console.log(image);

			$('h2').text(city_name);
			$(`<h3>- Country: ${ country }</h3>`).insertAfter('h2');
			$('div:first').append(`<h3>- Province: ${ province }</h3>`);
			$('div:first').append(`<img src = "${ image }" alt = "seoul">`);
